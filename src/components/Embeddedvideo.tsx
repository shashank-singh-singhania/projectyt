...
const Embeddedvideo = ({ id }: { id: string }) => {
  const [videos, setVideos] = useState<Video[]>([]);
  const [selectedVideo, setSelectedVideo] = useState<Video | null>(null);
  const router = useRouter();
//   const { id } = router.query; // Get video ID from URL
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3000/api/videos') // Replace with your API URL
      ...
      .then(data => {
        ...
        const video = data.Videos.find((v:any) => v._id === id);
  ...
  return (
    <div className="min-h-screen bg-black p-4">
      <div className="container mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-gray-200"><Link href={'/'}>Home</Link></h1>
